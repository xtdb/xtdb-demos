"""Production writers keep the outcome and availability timeline in one transaction."""
...
class FraudStatusWriterNodeTest(unittest.TestCase):
    ...
    def test_sim_initial_status_and_confirmation_start_at_the_write_time(self):
        with _connect() as conn:
            ...
            with conn.cursor() as cur:
                cur.execute("""SELECT is_fraud, _valid_from, _system_from
                               FROM fraud_status FOR ALL VALID_TIME
                ...
                self.assertEqual(cur.fetchone(), (True, transaction["ts"], D(40)))
    ...
    def test_retried_confirmation_uses_committed_time_and_replays_the_original_score(self):
        ...
        with psycopg.connect(dsn, autocommit=True) as conn, conn.cursor() as cur:
            ...
            cur.execute("SELECT _valid_from, _system_from FROM fraud_status WHERE _id = 'fraud'")
            self.assertEqual(cur.fetchone(), (D(31), D(31)))
            cur.execute("""SELECT COUNT(*) FROM fraud_status FOR ALL VALID_TIME WHERE is_fraud""")
            self.assertEqual(cur.fetchone()[0], 1)

    def test_backfill_writes_status_at_each_days_actual_availability(self):
        conn = _connect()
        dsn = conn.info.dsn
        ...
        import psycopg
        with psycopg.connect(dsn, autocommit=True) as conn, conn.cursor() as cur:
            cur.execute("SELECT COUNT(*) FROM fraud_status")
            self.assertEqual(cur.fetchone()[0], 600)
            cur.execute("SELECT COUNT(*) FROM fraud_status FOR ALL VALID_TIME WHERE is_fraud")
            self.assertGreater(cur.fetchone()[0], 0)
            cur.execute("""SELECT COUNT(*) FROM fraud_status FOR ALL VALID_TIME
                           WHERE _valid_from <> _system_from""")
            self.assertEqual(cur.fetchone()[0], 0)
            cur.execute("""SELECT COUNT(*) FROM label l LEFT JOIN fraud_status s ON l._id = s._id
                           WHERE s._id IS NULL OR s.is_fraud <> l.is_fraud""")
            self.assertEqual(cur.fetchone()[0], 0)


@unittest.skipUnless(_playground_up(), "needs playground on :5445")
class FraudStatusCompatibilityNodeTest(unittest.TestCase):
    def test_empty_dataset_is_compatible(self):
        with _connect() as conn, conn.cursor() as cur:
            model._require_fraud_status(cur)
    ...
    def test_partial_status_population_requires_a_fresh_seed(self):
        with _connect() as conn:
            with _learned_at(conn, D(1)) as cur:
                _txn(cur, "old", "a1", D(1))
                _txn(cur, "new", "a1", D(1))
                _label(cur, "new", D(1), False)
            with conn.cursor() as cur, self.assertRaisesRegex(RuntimeError, "fresh seed"):
                model._require_fraud_status(cur)

    def test_historical_status_is_sufficient_and_the_basis_limits_both_tables(self):
        with _connect() as conn:
            with _learned_at(conn, D(1)) as cur:
                _txn(cur, "old", "a1", D(1))
            ...
            with conn.cursor() as cur:
                model._require_fraud_status(cur)
                model._require_fraud_status(cur, system_time=D(0))
            with _learned_at(conn, D(3)) as cur:
                _txn(cur, "uncovered", "a1", D(3))
            with conn.cursor() as cur:
                model._require_fraud_status(cur, system_time=D(2))
